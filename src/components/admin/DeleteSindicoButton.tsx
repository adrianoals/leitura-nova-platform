...
import { deleteSindico } from '@/actions/sindicoActions';

type DeleteSindicoButtonProps = {
    sindicoId: string;
    returnPath?: string;
    compact?: boolean;
};
...
function SubmitButton({ compact, onClick }: { compact?: boolean; onClick: () => void }) {
    ...
    return (
        ...
        >
            ...
            {pending ? 'Excluindo...' : 'Excluir Síndico'}
        </button>
    );
}
...
export default function DeleteSindicoButton({ sindicoId, returnPath, compact }: DeleteSindicoButtonProps) {
    const [open, setOpen] = useState(false);

    return (
        <form action={deleteSindico}>
            <input type="hidden" name="sindico_id" value={sindicoId} />
            ...
            <ConfirmDeleteModal
                open={open}
                description="Tem certeza que deseja excluir este síndico? Isso excluirá o vínculo e poderá remover o acesso dele ao sistema."
            >
