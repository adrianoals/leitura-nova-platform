...
type Params = Promise<{ unidadeId: string; mes: string }>;
...
export default async function LeituraMesPage({ params }: { params: Params }) {
    const { unidadeId, mes } = await params;

    if (!isMesValido(mes)) {
        redirect(`/app/u/${unidadeId}/leituras`);
    }
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">{formatMes(mes)}</h1>
                    <p className="text-slate-500 text-sm">{formatUnidade(vinculo.unidade.bloco, vinculo.unidade.apartamento)}</p>
                </div>
            </div>
